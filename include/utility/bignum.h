...
public:
  ...
  // external method for testing simple_mod
  void mod(const Bignum& mod){
    Bignum const one(1);
    Digit big_data[2 * DIGITS];
    simple_mult(big_data, _data, one._data, DIGITS);

    simple_mod(_data, big_data, mod._data, DIGITS);
  }
...
private:
    ...
    _Word barrett_u(const BigNum& mod){
      ...
      // k could be calculated using floor(log(p ,b)) + 1
    ...
    // - res is assumed to be of size 'size'
    // - a is assumed to be of size '2*size'
    // - mod is assumed to be of size 'size'
    void simple_mod(Digit * res, const Digit * a, const Digit * mod, unsigned int size) {
        Digit q[size + 1];
        ...
        for(i = 0; i < size + 1; i++) {
            for(unsigned int j = 0; j <= i; j++) {
                unsigned int k = i - j;
                Double_Digit aj = q[j];
                Double_Digit bk = (k == size ? 0 : mod[k]);
                Double_Digit prod = aj * bk;
                r0 += Digit(prod);
                r1 += (prod >> BITS_PER_DIGIT) + (r0 >> BITS_PER_DIGIT);
                r0 = Digit(r0);
        ...
        // _data = r % _mod
        while((r[size] > 0) || (cmp(r, mod, size) >= 0)) {
            if(simple_sub(r, r, mod, size))
                r[size]--;
        }
